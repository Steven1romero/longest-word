...
export-env {
    ...
    let virtual_env = r#'/home/jsteven.romeror/code/Steven1romero/longest-word/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'longest-word-py3.12'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'longest-word-py3.12'#
    }
